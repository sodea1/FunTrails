                            ...
                            <div className="inner-dropdown-container">
                                ...
                                <div className="search-items-list">
                                    {Object.values(searchHash[this.state.filterBy]).map((entity, idx) => {
                                        if (entity.t_name ? entity.t_name : entity.p_name.toLowerCase().startsWith(this.state.search.toLowerCase())) {
                                            // debugger
                                            {let title = (entity.t_name) ? entity.t_name : entity.p_name }
                                            liveItemsList.push((entity.t_name) ? entity.t_name : entity.p_name)
                                            return (
                                                <Link to={`/trails/${entity.id}`} className="search-item" key={idx}>
                                                    <div className="loc-icon-div">
                                                        {(entity.t_name) ? <img className="loc-icon" src={window.green_loc} width="16px" height="22px" /> : <BsTree className="park-icon" height="40px" width="40px" />}
                                                    </div>
                                                    <div>
                                                        <h1>{(entity.t_name) ? entity.t_name : entity.p_name }</h1>
